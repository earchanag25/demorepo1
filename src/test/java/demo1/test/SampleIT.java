...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "project3-ear.ear");

        // Current directory is the root of the "project3-web" project. Go up one level
        // and enter the "project3-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../project3-ejb/target/project3-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "project3-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/project3-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "project3-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
